...
class ProgressiveNasalization : MatcherConditionPhonologicalChangeTendency() {
    override fun getNewInstance() = ProgressiveNasalization()

    override fun createMatchers(phonemeContainer: PhonemeContainer): List<List<PhonemeMatcher>> =
        listOf(
            createPhonemeMatchers("(C[+Nasal])(V[-Nasalized])", phonemeContainer)
        )
    ...
    override fun getOptions(language: Language, phonemes: PhonemeContainer): List<GenericSSO<PhonologicalRule>> =
        phonemes.createPhonologicalRulesFor(language) {
            return (createWeakRules("(C[+Nasal])(V[-Nasalized]) -> -[+Nasalized] / _ ") allWithProb 0.1) +
                    (createWeakRules("(V[-Nasalized]) -> [+Nasalized] / (C[+Nasal]) _ ") allWithProb 0.1)
        }
}
