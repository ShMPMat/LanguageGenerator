...
class ProgressiveVoicingAssimilation : MatcherConditionPhonologicalChangeTendency() {
    override fun getNewInstance() = ProgressiveVoicingAssimilation()
    ...
    override fun getOptions(language: Language, phonemes: PhonemeContainer): List<GenericSSO<PhonologicalRule>> =
        phonemes.createPhonologicalRulesFor(language) {
            return listOf(
                createRule("(C[+Voiced]) -> [-Voiced] / (C[-Voiced]) _ ") withProb 0.1,
                createRule("(C[-Voiced]) -> [+Voiced] / (C[+Voiced]) _ ") withProb 0.1
            )
        }
}
